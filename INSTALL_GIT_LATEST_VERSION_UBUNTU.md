# Compile and install git on Ubuntu 20.04
```shell
cd
...
sudo apt -y install build-essential autoconf   # install c compiler and tools
sudo apt -y install libz-dev libssl-dev libcurl4-gnutls-dev libexpat1-dev gettext cmake gcc # install packages
...
git --version  # get the version
sudo apt remove git  # removed the old version of git 
git --version  # get the version
...
source ~/.profile  # apply all changes defined in your .bashrc 
git --version # Check 
```
## Install git subtree
```shell 
